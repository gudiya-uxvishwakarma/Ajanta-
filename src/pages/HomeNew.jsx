...
function ProductCard({ product, showCountdown = false }) {
  ...
  return (
    ...
    >
      ...
      {showCountdown && (
        <motion.div 
          className="absolute top-3 left-3 right-3 z-10 bg-[#cc0000] px-3 py-1.5 rounded-lg"
      ...
      <div className="relative aspect-square overflow-hidden bg-gray-50" onClick={() => navigate(`/product/${card.id}`)}>
        ...
        {/* Icon Actions - Always visible on mobile, hover on desktop */}
        <motion.div 
        ...
        >
          ...
          <motion.button 
            className={`w-9 h-9 rounded-full flex items-center justify-center shadow-lg backdrop-blur-sm transition-all ${
              isWishlisted ? 'bg-[#cc0000] text-white' : 'bg-white/90 hover:bg-[#cc0000] hover:text-white'
            }`}
            whileTap={{ scale: 0.9 }}
            onClick={(e) => {
              e.stopPropagation();
          ...
          {/* Quick View Icon */}
          <motion.button 
            className="w-9 h-9 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg hover:bg-[#cc0000] hover:text-white transition-all"
            whileTap={{ scale: 0.9 }}
            onClick={(e) => {
              e.stopPropagation();
        ...
        >
          ...
          {/* Quick View Icon */}
          <motion.button 
            className="w-11 h-11 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg hover:bg-[#cc0000] hover:text-white transition-all"
            whileHover={{ scale: 1.15 }}
            whileTap={{ scale: 0.9 }}
            onClick={(e) => {
              e.stopPropagation();
